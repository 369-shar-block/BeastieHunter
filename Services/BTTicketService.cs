...
namespace BeastieHunter.Services
{
    public class BTTicketService : IBTTicketService
    {
        ...
        public async Task AssignTicketAsync(int ticketId, string userId)
        {
            ...
            try
            {
                if (ticket != null)
                {
                    try
                    {
                        ticket.DevloperUserId = userId;

                        ticket.TicketStatusId = (await LookupTicketPriorityIdAsync("Development")).Value;
                        await _context.SaveChangesAsync();
                    }
                    catch (Exception)
                    {
        ...
        public async Task<List<Ticket>> GetAllTicketsByCompanyAsync(int companyId)
        {
            try
            {
                List<Ticket> tickets = await _context.Projects.Where(p => p.CompanyId == companyId).SelectMany(p => p.Tickets).Include(t => t.Attachments)
        ...
        public async Task<List<Ticket>> GetTicketsByRoleAsync(string role, string userId, int companyId)
        {
            ...
            try
            {
                ...
                {
                    tickets = (await GetAllTicketsByCompanyAsync(companyId)).Where(t => t.DevloperUserId == userId).ToList();
                }
        ...
        public async Task<List<Ticket>> GetTicketsByUserIdAsync(string userId, int companyId)
        {
            ...
            try
            {
                ...
                {
                    tickets = (await _projectService.GetAllProjectsByCompany(companyId)).SelectMany(p => p.Tickets).Where(t => t.DevloperUserId == userId).ToList();
                }
                else if (await _rolesService.IsUserInRoleAsync(btUser, Roles.Submitter.ToString()))
                {
                    tickets = (await _projectService.GetAllProjectsByCompany(companyId)).SelectMany(t => t.Tickets).Where(t => t.OwnerUserId == userId).ToList();
                }
